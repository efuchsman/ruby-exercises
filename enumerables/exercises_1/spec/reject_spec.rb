RSpec.describe 'reject' do
  ...
  it 'remove words ending in e' do
    ...
    selected = words.reject {|word| word.chars.last == "e"}
    expect(selected).to eq(["you", "thinking", "belt", "sing"])
  end

  it 'remove words ending in ing' do
    words = ["bring", "finger", "drought", "singing", "bingo", "purposeful"]
    selected = words.reject {|word| word.chars.last(3) == ["i", "n","g"]}
  ...
  it 'remove numbers' do
    ...
    not_numbers = elements.reject {|element| element == element.to_i || element == element.to_f}
